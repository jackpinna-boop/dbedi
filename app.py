...
st.set_page_config(
    layout="wide",
    page_title="Dashboard Interventi – Prov. Sulcis Iglesiente"
)
...
ISTITUTI_FILENAME = "SCU_Istituti-ELE_ISTITUTI-2.csv"
INTERVENTI_FILENAME = "SCU_Istituti-ELE_CMPLSS.csv"
...
def df_riepilogo(df_source: pd.DataFrame) -> pd.DataFrame:
    if not {"codice", "determina_norm", "importo_stanziato"}.issubset(df_source.columns):
        return df_source
    return df_source.drop_duplicates(
...
# -------------------------------------------------------
# CARICAMENTO DATI (SENZA UPLOAD)
# -------------------------------------------------------
st.sidebar.subheader("📂 Dati caricati da cartella interna")
st.sidebar.write(f"Istituti: `data/{ISTITUTI_FILENAME}`")
st.sidebar.write(f"Interventi: `data/{INTERVENTI_FILENAME}`")

istituti = load_csv_from_repo(ISTITUTI_FILENAME, "ISTITUTI")
interventi = load_csv_from_repo(INTERVENTI_FILENAME, "INTERVENTI")
...
for col in ["codice", "nome_istituto", "comune"]:
    if col not in istituti.columns:
        st.error(
            f"File ISTITUTI: colonna '{col}' mancante. "
            f"Colonne trovate: {list(istituti.columns)}"
        )
        st.stop()
...
for col in ["codice", "denominazione_intervento", "determina", "manutenzioni", "tipologia_intervento"]:
    if col not in interventi.columns:
        st.error(
            f"File INTERVENTI: colonna '{col}' mancante. "
            f"Colonne trovate: {list(interventi.columns)}"
        )
        st.stop()
...
filtro_comune = st.sidebar.multiselect(
    "Comune",
    sorted(df["comune"].dropna().unique()),
)
...
if pagina == "Home":
    ...
    with col_g2:
        ...
        st.bar_chart(
            pd.DataFrame(
                {"Tipo": ["Manutenzioni", "Altri"], "Valore": [n_m, n_a]}
            ).set_index("Tipo")
        )
    ...
    if "importo_stanziato" in df_filt.columns:
        ...
        with col_e2:
            st.markdown(
                "**Somma importi per tipologia (dedup determina/importo per istituto)**"
            )
            s_tip = (
        ...
        # Istituti coinvolti per tipologia
        ...
        df_acc = df_filt[
            df_filt["tipologia_intervento"].str.lower() == "accordo/servizio"
        ].copy()
        ...
        if df_acc.empty:
            st.info(
                "Nessuna determina per la tipologia 'Accordo/Servizio' nei filtri correnti."
            )
        else:
            df_acc["determina_norm"] = (
                df_acc["determina"].astype(str).str.strip().str.lower()
            )
            ...
            det_acc["importo_per_scuola"] = (
                det_acc["importo_stanziato"] / det_acc["numero_scuole"]
            )

            det_acc["Importo totale (€)"] = det_acc["importo_stanziato"].map(fmt_eur)
            det_acc["Quota per scuola (€)"] = det_acc["importo_per_scuola"].map(
                fmt_eur
            )

            st.dataframe(
                det_acc[
                    ["determina", "numero_scuole", "Importo totale (€)", "Quota per scuola (€)"]
                ],
                use_container_width=True,
            )
            ...
            st.success(
                f"Totale complessivo quote Accordo/Servizio (somma quote per scuola): {fmt_eur(tot_quota)}"
            )
        ...
        st.success(
            f"**Totale generale stanziato (con quote manutenzioni): {fmt_eur(totale_generale)}**"
        )
...
# -------------------------------------------------------
    ...
    with cg2:
        ...
        st.bar_chart(
            pd.DataFrame(
                {"Tipo": ["Manutenzioni", "Altri"], "Valore": [n_mi, n_ai]}
            ).set_index("Tipo")
        )
    ...
    if REPORTLAB_AVAILABLE:
        def crea_pdf(data: pd.DataFrame, nome: str) -> BytesIO:
            ...
            if "importo_stanziato" in data_local.columns:
                data_local["determina_norm"] = (
                    data_local["determina"].astype(str).str.strip().str.lower()
                )
            ...
            if "importo_stanziato" in data_rip_ist.columns:
                ...
                if not df_acc_all.empty:
                    df_acc_all["determina_norm"] = (
                        df_acc_all["determina"]
                        .astype(str)
                        .str.strip()
                        .str.lower()
                    )
                    ...
                    det_acc_all["importo_per_scuola"] = (
                        det_acc_all["importo_stanziato"]
                        / det_acc_all["numero_scuole"]
                    )
                    ...
                    df_acc_ist = det_acc_merge[
                        det_acc_merge["nome_istituto"] == nome
                    ]
            ...
            if "importo_stanziato" in data_rip_ist.columns:
                ...
                s_al_raw = data_rip_ist[~data_rip_ist["manut_flag"]][
                    "importo_stanziato"
                ].sum()
            ...
            for _, row in data_local.iterrows():
                if (
                    "importo_stanziato" in row
                    and pd.notna(row["importo_stanziato"])
                ):
        ...
        st.info(
            "Generazione PDF disabilitata: modulo 'reportlab' non disponibile."
        )
